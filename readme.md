...
我最近在玩用扩散模型做图像编辑，用的模型是FLUX.2-klein，但是图像编辑后总是有色差，于是大家会用图像颜色匹配算法，把颜色矫正回来。

但实际上还是有问题，编辑的幅度比较大时<sub>(比如删除了面积大的物体)</sub>，传统算法并不能很好地匹配颜色，还是会有色差。
...
收敛不是肯定最快的，不过速度还算能接受，在4060ti上也只需要运行10秒。听起来有点慢，但是考虑到是做扩散模型的后处理，其实也足够快以至于我没有什么再优化的动力了！
